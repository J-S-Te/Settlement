...
已提供独立 Go API、Worker 与统一前端模块的可运行闭环：合同财务事件 Inbox、应收计划确认、正式应收、回款登记、余额受控的回款核销/冲销、开票申请/审批/手工登记、账龄催收、应收账龄 TOP10、异步 CSV 报表导出、本地通知，以及审计和高优先级通知 Outbox。运行入口为 `go run ./cmd/api`、`go run ./cmd/worker` 和 `go run ./cmd/migrate`。
...
当前上线前风险：登出仅撤销结算本地会话，尚未调用 OIDC `end_session_endpoint`；登录登出路由需要按生产安全门禁限制方法和来源；发票可开额度、租户安全 JOIN 和真实税控/银行/ERP Provider 仍需专项验收。

生产启用税控前需配置真实适配器 URL、令牌地址和 Client Credentials；未配置时仍可走审批后的手工发票登记，不影响应收、回款与核销主链路。
...
| GET/POST | `/api/v1/invoice-requests`、`/api/v1/invoice-requests/{id}/approve` | 发票申请与审批 |
