...
const (
	InvoicedReceivablesCSV = "INVOICED_RECEIVABLES_CSV"
	AgingReceivablesCSV    = "AGING_RECEIVABLES_CSV"
)
...
type job struct{ id, tenantID, reportType string }
...
func (w *Worker) RunOnce(ctx context.Context) error {
	...
	rows, err := tx.QueryContext(ctx, `SELECT id,tenant_id,report_type FROM settlement_report_export_job WHERE report_type IN (?,?) AND (status='PENDING' OR (status='PROCESSING' AND locked_until<UTC_TIMESTAMP(3))) ORDER BY created_at ASC LIMIT ? FOR UPDATE SKIP LOCKED`, InvoicedReceivablesCSV, AgingReceivablesCSV, limit)
	if err != nil {
		_ = tx.Rollback()
		return err
	}
	...
	for rows.Next() {
		var item job
		if err := rows.Scan(&item.id, &item.tenantID, &item.reportType); err != nil {
			rows.Close()
			_ = tx.Rollback()
			return err
		}
...
func (w *Worker) generate(ctx context.Context, item job) {
	var content []byte
	var err error
	filenamePrefix := "invoiced-receivables"
	if item.reportType == AgingReceivablesCSV {
		content, err = w.agingCSV(ctx, item.tenantID)
		filenamePrefix = "aging-receivables"
	} else {
...
func (w *Worker) csv(ctx context.Context, tenantID string) ([]byte, error) {
	rows, err := w.DB.QueryContext(ctx, `SELECT r.receivable_no,cs.source_contract_no,cs.customer_name_snapshot,DATE_FORMAT(r.due_date,'%Y-%m-%d'),r.original_amount,r.open_amount,r.currency,r.collection_status,r.invoice_status FROM settlement_receivable r JOIN settlement_contract_snapshot cs ON cs.id=r.contract_snapshot_id AND cs.tenant_id=r.tenant_id WHERE r.tenant_id=? AND r.recognition_status='CONFIRMED' AND r.open_amount>0 AND r.invoice_status<>'NOT_INVOICED' ORDER BY r.open_amount DESC,r.due_date ASC,r.id ASC LIMIT 10000`, tenantID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		values[7] = collectionStatusLabel(values[7])
		values[8] = invoiceStatusLabel(values[8])
...
func (w *Worker) agingCSV(ctx context.Context, tenantID string) ([]byte, error) {
	rows, err := w.DB.QueryContext(ctx, `SELECT r.receivable_no,cs.source_contract_no,cs.customer_name_snapshot,DATE_FORMAT(r.due_date,'%Y-%m-%d'),r.original_amount,r.open_amount,r.currency,GREATEST(TIMESTAMPDIFF(DAY,r.due_date,UTC_DATE()),0),CASE WHEN r.due_date>=UTC_DATE() OR TIMESTAMPDIFF(DAY,r.due_date,UTC_DATE())<=30 THEN '0-30' WHEN TIMESTAMPDIFF(DAY,r.due_date,UTC_DATE())<=60 THEN '31-60' WHEN TIMESTAMPDIFF(DAY,r.due_date,UTC_DATE())<=90 THEN '61-90' ELSE '90+' END,r.collection_status,r.invoice_status FROM settlement_receivable r JOIN settlement_contract_snapshot cs ON cs.id=r.contract_snapshot_id AND cs.tenant_id=r.tenant_id WHERE r.tenant_id=? AND r.recognition_status='CONFIRMED' AND r.open_amount>0 ORDER BY r.due_date ASC,r.id ASC LIMIT 10000`, tenantID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	buffer.Write([]byte{0xEF, 0xBB, 0xBF})
	writer := csv.NewWriter(&buffer)
	if err := writer.Write([]string{"应收单号", "合同号", "客户", "到期日", "原始应收金额", "未回款金额", "币种", "逾期天数", "账龄区间", "回款状态", "开票状态"}); err != nil {
		return nil, err
	}
	for rows.Next() {
		...
		values[8] = map[string]string{"0-30": "0–30 天", "31-60": "31–60 天", "61-90": "61–90 天", "90+": "90 天以上"}[values[8]]
		values[9] = collectionStatusLabel(values[9])
		values[10] = invoiceStatusLabel(values[10])
		if err := writer.Write(values[:]); err != nil {
			return nil, err
		}
	}
...
func collectionStatusLabel(value string) string {
	return map[string]string{"UNPAID": "未回款", "PARTIALLY_SETTLED": "部分回款", "SETTLED": "已结清"}[value]
}

func invoiceStatusLabel(value string) string {
